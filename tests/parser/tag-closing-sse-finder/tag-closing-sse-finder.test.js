...
function findClosingTagPosition(content, startPos, closingTag) {
  ...
  while (scanPos < content.length) {
    ...
    if (
      inHtmlComment &&
      char === "-" &&
      content.substring(scanPos, scanPos + 3) === "-->"
    ) {
      inHtmlComment = false;
      scanPos += 3;
      continue;
    ...
    if (
      ...
      if (
        content
          .substring(scanPos, scanPos + closingTag.length)
          .toLowerCase() === closingTag.toLowerCase()
      ) {
        return scanPos;
      }
    }
...
const result1 = findClosingTagPosition(
  test1,
  test1.indexOf("<content>") + 9,
  "</write_to_file>",
);
console.log("\n✅ Test 1: Simple case");
console.log(`   Expected: Find real tag at end`);
console.log(
  `   Result: ${result1 !== -1 ? "PASS" : "FAIL"} (found at ${result1})`,
);
...
console.log(
  `   Result: ${result2 === expected2 ? "PASS ✅" : "FAIL ❌"} (found at ${result2})`,
);
...
console.log(
  `   Result: ${result3 === expected3 ? "PASS ✅" : "FAIL ❌"} (found at ${result3})`,
);
...
console.log(
  `   Result: ${result4 === expected4 ? "PASS ✅" : "FAIL ❌"} (found at ${result4})`,
);
...
console.log(
  `   Result: ${result5 === expected5 ? "PASS ✅" : "FAIL ❌"} (found at ${result5})`,
);
...
console.log(
  `   Result: ${result6 === expected6 ? "PASS ✅" : "FAIL ❌"} (found at ${result6})`,
);
