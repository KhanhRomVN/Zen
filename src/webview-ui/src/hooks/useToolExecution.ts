 ...
 * This is a conservative limit — most models allow 128k tokens but we warn
...
function applyTokenLimitGuard(
  ...
  // Log token breakdown for every autoReq flush
  const overflowProneBreakdown = actions
    .filter((a) => OVERFLOW_PRONE_TOOLS.has(a.type))
    .map((a, i) => {
      const r = results[i] || "";
