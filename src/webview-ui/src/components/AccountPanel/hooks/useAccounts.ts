  ...
  const fetchAccounts = useCallback(
    async (page = 1, limit = 20, silent = false) => {
      ...
      try {
        ...
        if (providerFilter && providerFilter !== "") params.append("provider_id", providerFilter);
        ...
        if (result.success && result.data) {
          ...
          const accountsWithDailyStats = await Promise.all(
            accountsList.map(async (acc: any) => {
              try {
                const statsResult = await callBackend(`/v1/stats?period=day&account_id=${acc.id}`);
                let dailyTokens = 0;
                let dailyRequests = 0;
                
                if (statsResult.success && statsResult.data?.usage) {
                  // Sum all tokens and requests from hourly usage
                  dailyTokens = statsResult.data.usage.reduce((sum: number, hour: any) => sum + (hour.tokens || 0), 0);
                  dailyRequests = statsResult.data.usage.reduce((sum: number, hour: any) => sum + (hour.requests || 0), 0);
                }
                ...
                console.error(`Failed to fetch stats for account ${acc.id}:`, err);
                return {
                  id: acc.id,
                  provider_id: acc.provider_id,
                  email: acc.email,
    ...
    [isOpen, searchQuery, providerFilter, emailFilter, providerConfigs.length, callBackend]
  );
